		...
		function handleArrowKey(event: KeyboardEvent) {
			...
			switch (event.key) {
				...
				case 'Tab':
					...
					if (direction === 'down') {
						let nextColumn = focused.column + 1;
						if (nextColumn >= totalColumns) nextColumn = 0;
...
<h2 class="m-auto capitalize">{direction}</h2>
...
<div class="flex justify-center gap-10">
	...
	<div>
		<h2>Across</h2>
		<ul>
			{#each $crossword.clues.across as clue}
		...
		<h2>Down</h2>
		<ul>
			{#each $crossword.clues.down as clue}
				<li class="my-1 flex items-center gap-2">
					<span class="w-6 text-right font-semibold">{clue.number}.</span>
					{clue.clue}
				</li>
